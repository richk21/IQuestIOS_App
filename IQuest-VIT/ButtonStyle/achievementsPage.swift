...
struct achAnimatedBackground : View {
    @State var start = UnitPoint(x: 0, y: -2)
    @State var end = UnitPoint(x: 4, y: 0)
    let timer = Timer.publish(every: 1, on: .main, in: .default).autoconnect()
    let color = [Color.black, Color.yellow]
    var body : some View {
...
struct achievementsPage: View {
    ...
    var body: some View {
        ZStack{
            achAnimatedBackground().edgesIgnoringSafeArea(.all)
                .zIndex(0)
            ...
            VStack{
                ...
                ScrollView(.vertical, showsIndicators: false){
                    ...
                    VStack(spacing: 30*(896/height)){
                        ForEach(AchievementsViewController.AchievDict){item in
                            GeometryReader{ reader in
                                
                                //expandView(isExpand: $isExpanding, data: item, activeId: $activeId)
                                expandViewAch(isExpand: $isExpanding, activeId: $activeId, data: item)
...
struct expandViewAch: View{
    
    @Binding var isExpand : Bool
    @Binding var activeId : UUID
    var data: achieveCard
    @State var url = ""
